...
echo "Xong. Deploy từ gốc repo (Root Directory trên Railway để trống + RAILWAY_DOCKERFILE_PATH):"
echo "  cd \"$ROOT\" && railway up . --service api -e production -p $PROJECT_ID"
echo "  cd \"$ROOT\" && railway up . --service web -e production -p $PROJECT_ID"
echo "  Hoặc: ./scripts/railway-provision.sh"
